...
const EditModal = ({item, remove, dispatch}) => {
    ...
    const writeDB = async () => {
        ...
        try {
            ...
            reduxDispatch(getError(`${e.message} : ${e.response.data}`))
        }
    }
    const saveOptions = () => {
        setEditable(false)
            ...
            <div onClick={event => event.stopPropagation()} className="item__container">
                ...
                <div className="item__content">
                    ...
                    {isEditable && <div className="item__options">
                        <div className="item__title">
                            <h2>Title:</h2>
                            <input onChange={(e) => editTitle(e.target.value)} className='item__title-edit'
                                   value={newItem.title || newItem.name || ''} type="text"/>
                        </div>
                        <div className="item__overview">
                            <h2>Overview:</h2>
                            <textarea onChange={(e) => setItem({...newItem, overview: e.target.value})}
                                      className='item__overview-edit' value={newItem.overview}></textarea>
                        </div>
                        <div className="item__adult">
                            <h2>Adult:</h2>
                            <input onChange={event => setItem({...newItem, adult: event.target.checked})}
                                   checked={newItem.adult} type="checkbox"/>
                        </div>
                        <div className="item__video">
                            <h2>Video:</h2>
                            <input onChange={event => setItem({...newItem, video: event.target.checked})}
                                   checked={newItem.video} type="checkbox"/>
                        </div>
